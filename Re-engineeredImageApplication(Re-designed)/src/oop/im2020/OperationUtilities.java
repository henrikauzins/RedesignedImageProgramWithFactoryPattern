...
public class OperationUtilities {
	// operations that do the low level work for the image operations
    ...
    public static int threshold(int rgb, double alpha) {
        int[] rgbA = getRGB(rgb);
        int sum = 0;
        for (int i = 0; i < rgbA.length; i++) {
            sum += rgbA[i];
    ...
    // tints image with a selection of colours and intensity
    ...
    // blends two images together with a chosent alpha value
